...
---@return table @The new instance
function PlayerMovementCorrectionEvent.new(player, vehicle, coordinatesOnVehicle, forwardSpeed)
    ...
    assert(coordinatesOnVehicle, "Coordinates are nil in PlayerMovementCorrectionEvent")
...
function PlayerMovementCorrectionEvent:run(connection)
    ...
    if not connection:getIsServer() then
        ...
        g_server:broadcastEvent(PlayerMovementCorrectionEvent.new(self.player, self.vehicle, self.coordinatesOnVehicle, self.forwardSpeed), nil, connection, self.player)
    end
end